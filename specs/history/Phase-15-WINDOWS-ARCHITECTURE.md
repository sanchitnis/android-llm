...
## 6. Security and Privacy Sandboxing

All security sandboxing configurations are applied as final implementation steps in Sprint 13 to guarantee user privacy and offline data containment.

### 6.1 User SID-Based Filesystem Sandboxing
To prevent local multi-user data leakage on shared Windows systems:
- **Directory Protection**: On launch, the application configures Windows Access Control Lists (ACLs) on the `%LOCALAPPDATA%\AdmissionCounselor` folder.
- **Access Control Entries (ACE)**: Using Windows security APIs, it removes permission inheritance and grants `Full Control` exclusively to the current user's Security Identifier (Owner SID) and `NT AUTHORITY\SYSTEM`. All other user SIDs are blocked from reading or writing to the directory.

### 6.2 Windows DPAPI API Key Encryption
The Priority 2 Gemini API Key is never stored in plaintext on disk:
- **Dynamic Encryption Wrapper**: The application calls native Windows Data Protection API (DPAPI) via JNI wrappers:
  - `CryptProtectData` encrypts the API key using the logged-in user's Windows credentials.
  - `CryptUnprotectData` decrypts the key byte array in-memory during inference initialization.
- **Plaintext Storage Prevention**: The resulting cipher bytes are Base64 encoded and written to `%LOCALAPPDATA%\AdmissionCounselor\config\settings.json`.

### 6.3 Volatile Transcript Memory Policy
Conversation logs and query transcripts must exist exclusively in volatile memory:
- **In-Memory Volatility**: The ViewModels store conversation logs in Kotlin StateFlow instances and wipe them on idle timeouts or application close.
- **JVM String Clearing**: To prevent garbage collector delays from leaving plaintext strings in memory, prompt construction uses `CharArray` buffers. Once processed, these character arrays are immediately zeroed out:
  ```kotlin
  promptBuffer.fill('\u0000')
  ```

### 6.4 Outbound Traffic Restrictions
To maintain the offline containment boundary:
- **Outbound Restrictions**: A custom Windows Firewall inbound/outbound rule is specified for packaging.
- **Network Containment**: The firewall configuration restricts `AdmissionCounselor.exe` to only query `generativelanguage.googleapis.com` (port 443) when using the Priority 2 Gemini API client. All other outbound network requests, telemetry loops, or diagnostic logs are blocked.

---

## 7. UI Layout and Window Lifecycle UX

The desktop interface is built using Compose Multiplatform, adapting the Android layout assets to desktop window constraints and host lifecycle events.

### 7.1 Desktop Window Layout
The main window utilizes a responsive dual-pane structure:
- **Collapsible Sidebar (Left Pane, 260dp Width)**: Houses active in-memory chat session titles, an "Add New Chat" action button, and a settings shortcut gear.
- **Chat Pane (Right Pane, Expanding)**: Features the conversation message stream showing alternating chat bubbles, a bottom-anchored text input field with send buttons, and dynamic system status banners (e.g., warning user when fallback CPU mode is active due to slow generation speeds).

### 7.2 Desktop Settings Dialog
A dedicated settings overlay provides local folder resolution and API configurations:
- **Weights Location Picker**: Displays a text field with the resolved model path and a "Browse" button. Clicking this triggers the native platform `AWT FileDialog` in folder-select mode to retrieve the path.
- **Inference Priority Diagnostics**: Displays a read-only diagnostics indicator detailing the active execution engine type (Local GPU / Cloud API / Local CPU Fallback).
- **Secure Key Entry**: Displays a password-masked input text field for the Gemini API Key. Clicking "Save" encrypts the string via the DPAPI JNI bindings.

### 7.3 Brand Theme Integration
To align with the REVA University design requirements:
- **Colors**:
  - Primary UI elements, progress circles, and user message bubbles use REVA Orange `#f7a35b`.
  - Sidebar background, inactive buttons, and top toolbars use REVA Grey `#4a4c55`.
- **Fonts**:
  - Headers and title bars use `Plus Jakarta Sans`.
  - Conversation messages and form inputs use `Glacial Indifference` (using Desktop JRE resource packaging for fonts loading).

### 7.4 Window State Lifecycle Listeners
Compose Multiplatform exposes application-level window listeners linked to engine lifecycles:
- **Minimized state listener (`isMinimized` == true)**: If the window is minimized or remains hidden for over 30 seconds, the app invokes `LlmInferenceJni.close()` to free up CPU threads and native VRAM.
- **Restoration state listener (`isMinimized` == false)**: Returning the window triggers a background task to pre-warm the engine options and verify the weights handle, minimizing response delay.
- **Window Close Hook (`onCloseRequest`)**: Clicking the window close button disposes the active database connection, runs native JNI engine release callbacks, clears volatile in-memory StateFlow lists, and terminates the JVM process cleanly.
