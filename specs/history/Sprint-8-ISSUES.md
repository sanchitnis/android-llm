...
| ISS-021 | HIGH | Cancel unloads the model from RAM causing a cold start loading delay for subsequent messages. | CLOSED | Closed engine to release C++ thread, but added proactive background re-initialization immediately after stop to pre-warm the engine. |
| ISS-022 | MEDIUM | Throughput log uses callback counts rather than actual tokens, leading to underreported tokens/sec. | CLOSED | Updated throughput log to estimate tokens using standard character-based heuristic (`length / 4`) at completion. |
| ISS-023 | HIGH | maxTokens set to 1546 in LlmEngine.kt, restricting the total context window rather than output generation budget. | CLOSED | Increased maxTokens in LlmInferenceOptions to 4096 to match the PRD context window budget. |
| ISS-024 | HIGH | Single 2.58 GB install-time asset pack exceeds Google Play's 1.5 GB individual asset pack size limit. | CLOSED | Adopted direct file sideloading strategy to bypass Google Play size ceilings and avoid storage duplication. |
| ISS-025 | HIGH | LlmThread executor destroyed in ViewModel.onCleared(), causing RejectedExecutionException on async close/cleanup jobs. | CLOSED | Removed LlmThread.destroy() from ViewModel.onCleared() to keep the process-level thread executor alive. |
| ISS-026 | HIGH | WebGPU fallback constraints: Devices lacking WebGPU support will experience extremely slow CPU-based inference. | CLOSED | WebGL 2.0 and Web Worker CPU execution defined as fallbacks in PWA-ARCHITECTURE.md, with UI alerts indicating degraded performance. |
| ISS-027 | MEDIUM | OPFS storage quota limits: Mobile browsers may restrict storage space below the 5.0 GB minimum required for model cache. | CLOSED | Added pre-flight storage estimation check (`navigator.storage.estimate()`) in PWA-ARCHITECTURE.md to verify available disk space. |
