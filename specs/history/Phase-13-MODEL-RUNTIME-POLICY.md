...
Prolonged model generation generates heat. The app monitors device thermal state via `PowerManager.getThermalStatus()` (API 29+), which is available to all standard Play Store apps without special privileges.

> **Note**: `HardwarePropertiesManager.getDeviceTemperatures()` is NOT used because it is restricted to system apps and device owner apps since Android 10. Standard Play Store apps would receive a `SecurityException`.

### 4.1 Thermal State Policies

| `PowerManager` Thermal Status | Engine Policy Action | Expected Impact |
| :--- | :--- | :--- |
| **`THERMAL_STATUS_NONE` / `THERMAL_STATUS_LIGHT`** | Run full speed with GPU acceleration and speculative decoding. | Standard performance: 15+ tokens/second. |
| **`THERMAL_STATUS_MODERATE`** | Introduce a 40ms pause delay between each generated token. | Reduces GPU utility by ~25% to control temperature rise. |
| **`THERMAL_STATUS_SEVERE` or higher** | - Disable speculative decoding. - Switch backend from GPU (ML Drift) to CPU (XNNPACK). | Throughput drops to ~5 tokens/second, but heat generation drops by 60%. |

### 4.2 Thermal Listener Registration
```kotlin
val powerManager = context.getSystemService(PowerManager::class.java)
powerManager.addThermalStatusListener { status ->
    when (status) {
        PowerManager.THERMAL_STATUS_NONE,
        PowerManager.THERMAL_STATUS_LIGHT -> engine.setThermalPolicy(ThermalPolicy.FULL_SPEED)
        PowerManager.THERMAL_STATUS_MODERATE -> engine.setThermalPolicy(ThermalPolicy.THROTTLED)
        else -> engine.setThermalPolicy(ThermalPolicy.DEGRADED)
    }
}
```
